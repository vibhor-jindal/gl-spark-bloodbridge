...
export const donorApi = {
  ...
  get: (donorId: number) => client.get<DonorResponse>(`/api/donors/${donorId}`).then((r) => r.data),
  search: (bloodGroup: string, city: string) =>
    client
      .get<DonorResponse[]>(`/api/donors/search/${encodeURIComponent(bloodGroup)}`, { params: { city } })
      .then((r) => r.data),
